...
function OrderTable({ columns, data, keywords }) {
  ...
  const handleSubmit = async (event, id) => {
    ...
    if (validate()) {
      ...
      axios({
        method: "PUT",
        url: `https://lacha.s2tek.net/api/Garden/editStatus/${id}`,
        data: formData,
        headers: {
          ...
          {OrderList &&
            OrderList.map((item, index) => (
              <tr key={index}>
                ...
                <td>
                  {item.status == "1" ? (
                    <LabelDra>Đang xử lí</LabelDra>
                  ) : item.status == "2" ? (
                    <LabelAct>Đang thuê</LabelAct>
                  ) : item.status == "3" ? (
                    <LabelDr>Đã hủy</LabelDr>
                  ) : null}
                </td>
                <td>
                  <Popup
                        ...
                        <form onSubmit={(e) => handleSubmit(e, item.id)}>
                          <div className="content">
                            ...
                            <select
                              ...
                              // id="status"
                              name="status"
                              value={items.status ? items.status : item.status}
                          ...
                          <div style={{ display: "flex", marginTop: "90px" }}>
                            ...
                            <LabelDz>
                              <button onClick={() => (close(),setItems({ id: orderId, status: "" }))}>Cancel</button>
                            </LabelDz>
                          </div>
                        </form>
                      </div>
